...
function descargarPlantilla() {
  const csv = generarPlantillaCSV(
    ["nombre", "nivelAcademico", "telefono", "correo"],
    ["Sánchez Domínguez Ana", "Primaria", "9991234567", "ana.sanchez@gardner.mx"]
  );
...
export default function CargarDocentesClient() {
  ...
  function onArchivo(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => {
      try {
        ...
        const objetos = filasComoObjetos<FilaImportacionDocente>(csv, ALIASES).filter((f) => f.nombre);
        if (objetos.length === 0) {
  ...
  function importar() {
    ...
    startTransition(async () => {
      const res = await importarDocentesAction(filas);
      if (!res.ok) {
        setError(res.error);
        return;
      }
    ...
    <div className="flex flex-col gap-6">
      ...
      {filas.length > 0 && !resultados && (
        <div className="flex flex-col gap-3">
          <p className="text-sm font-semibold text-gardner-gris">Vista previa — {filas.length} docentes detectados</p>
              ...
              <tbody>
                {filas.slice(0, 50).map((f, i) => (
                  <tr key={i} className="border-t border-gardner-gris/15">
                    ...
                    <td className="px-4 py-2 text-gardner-gris/70">{f.nivelAcademico || "—"}</td>
          ...
          <div className="flex gap-2">
            ...
            >
              {isPending ? "Importando…" : `Importar ${filas.length} docentes`}
            </button>
            <button
              onClick={reiniciar}
